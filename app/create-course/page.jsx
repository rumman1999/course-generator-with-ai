...
import { HiMiniSquares2X2 , HiMiniQueueList , HiDocumentDuplicate   } from "react-icons/hi2";
            ...
            {StepperOption.map((item, index) => (
              <div className="flex items-center">
                <div className="flex flex-col items-center w-[50px] md:w-[100px]">
                  <div
                    className={`bg-gray-950 p-3 rounded-full text-white ${
                      activeIndex >= index && `bg-gray-700 text-gray-50`
                    }`}
                  >
                    {item.icon}
                  </div>
                  <h2 className="hidden md:block md:text-sm text-gray-200 mt-2">{item.name}</h2>
                </div>
